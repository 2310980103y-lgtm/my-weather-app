...
if menu == "🏠 ホーム":
    ...
    c1.metric("最高", f"{res['daily']['temperature_2m_max'][idx]}°")
    c2.metric("雨率", f"{res['daily']['precipitation_probability_max'][idx]}%")
    c3.metric("最低", f"{res['daily']['temperature_2m_min'][idx]}°")
    ...
    st.session_state.time_evening = st.time_input("晩", st.session_state.time_evening)
    if st.button("通知記録リセット"):
        st.session_state.history = {"朝": False, "昼": False, "晩": False}
        st.success("リセット完了")
